...
def adsr(duration, attack, decay, sustain, release):
    ...
    limits = map(lambda x: x/1000, [0, attack, attack+decay, duration-release, duration])
    pairs = list(nwise(limits))
    
    return lambda xs: np.piecewise(
        xs*1000,
        [(xs >= s)&(xs<e) for s,e in pairs],
        [a,d,s,r,default]
    )
...
def play_sequence(notes, note_duration, offset, sample_rate=44100):
    ...
    duration = (n-1)*offset + note_duration + .25
    ...
    envelope = adsr(note_duration*1000, 90, 50, .8, 25)

    samples = np.sum([s(xs) * envelope(xs-i*offset) for i,s in enumerate(sounds)], 0)
